...
void main() {
  group('DesktopUpdateService Version Comparison Tests', () {
    test('Correctly compares higher and lower semver strings', () {
      expect(DesktopUpdateService.compareVersions('1.0.1', '1.0.0'), greaterThan(0));
      expect(DesktopUpdateService.compareVersions('1.1.0', '1.0.9'), greaterThan(0));
      expect(DesktopUpdateService.compareVersions('2.0.0', '1.99.99'), greaterThan(0));
      expect(DesktopUpdateService.compareVersions('1.0.0', '1.0.1'), lessThan(0));
      expect(DesktopUpdateService.compareVersions('0.9.9', '1.0.0'), lessThan(0));
    });

    test('Handles v prefix and build metadata gracefully', () {
      expect(DesktopUpdateService.compareVersions('v1.0.0', '1.0.0'), equals(0));
      expect(DesktopUpdateService.compareVersions('v1.2.0', 'v1.1.9'), greaterThan(0));
      expect(DesktopUpdateService.compareVersions('1.0.0+1', '1.0.0+2'), equals(0));
    });
  });
  ...
  group('DesktopUpdateService Asset Selection Tests', () {
    test('Finds matching asset from assets list', () {
      final assets = [
        {'name': 'flanki-1.1.0-mac.dmg', 'browser_download_url': 'https://download/mac.dmg'},
        {'name': 'flanki-setup-1.1.0-windows.exe', 'browser_download_url': 'https://download/windows.exe'},
        {'name': 'flanki-1.1.0-linux.AppImage', 'browser_download_url': 'https://download/linux.AppImage'},
      ];
  ...
  group('DesktopUpdateService API Check Tests', () {
    ...
    test('Returns hasUpdate=false when current version is latest or newer', () async {
      final mockResponse = {
        'tag_name': 'v1.0.0',
