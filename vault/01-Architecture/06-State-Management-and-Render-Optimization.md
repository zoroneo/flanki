...
|---|---|---|---|---|
| 1 | `AuthState` | `lib/core/auth/auth_state.dart` | `auth_state.freezed.dart` | `unauthenticated()`, `authenticated()`, `error()`, deep equality |
...
2. **Functional Providers**:
   - Khai báo gọn gàng bằng `@Riverpod(keepAlive: true) T myService(Ref ref)`.
3. **Lợi ích kiến trúc**:
...
## 5. Quy Chuẩn Kiểm Định (Quality Gates)
...
2. `fvm flutter test` chạy trọn vẹn toàn bộ 117 tests không lỗi.
